    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          '«Массовые теги» открывает широкую popup-панель над таблицей. У каждого поля есть режим «Не менять / Заменить / Очистить»; разные исходные значения явно помечаются.',
          '«Преобразования» вынесены в соседнюю popup-панель: trim краёв, верхний/нижний регистр, поиск/замена, префикс, суффикс и Artist ↔ Album Artist. Для преобразований Preview обязателен перед Apply.',
          'Обе панели работают с текущим выделением, автозакрываются после ухода курсора и используют существующий общий Undo change-set.',
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'Batch tags opens a wide popup above the table. Every field has Keep / Replace / Clear mode and mixed source values are clearly marked.',
          'Transforms uses a neighboring popup for trim, upper/lower case, find/replace, prefix, suffix and Artist ↔ Album Artist. Transform Preview is mandatory before Apply.',
          'Both panels operate on the current selection, auto-close after the pointer leaves, and use the existing single Undo change set.',
