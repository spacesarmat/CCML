    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'Hotfix 19.2.1 исправляет файлы, которые встроенный AudioLoader Essentia ошибочно считает тишиной после ошибок декодирования MP3/AAC. При сообщениях AudioLoader/Error reading frame/«completely silent file» CCML один раз декодирует источник своим настроенным FFmpeg во временный mono PCM WAV 44.1 kHz и повторяет Essentia. Исходный файл не изменяется; временный WAV удаляется после анализа.',
          'Stage 19.3 добавляет DJ-инструменты библиотеки: отдельные сортируемые колонки BPM, Musical Key и Camelot/Open Key, отдельный Camelot-фильтр и обновлённый DJ-пресет таблицы.',
          'В Inspector → Анализ показываются BPM, исходная тональность, Camelot/Open Key и консервативный набор совместимых ключей: текущий Camelot, соседние номера на той же стороне и relative major/minor на противоположной стороне. «Найти совместимые» очищает текстовый поиск, применяет этот Camelot-набор к полной библиотеке и сортирует результат по Camelot → BPM.',
          'Stage 19.4 ускоряет Essentia: по умолчанию Fast анализирует центральные 120 секунд длинного трека через FFmpeg, а очередь обрабатывает 2 трека параллельно. В Настройках доступны Fast/Accurate, 1–4 worker-а и длина Fast-фрагмента 30–300 секунд.',
          'Массовый анализ по умолчанию пропускает файлы, у которых сохранённый Essentia-анализ новее времени изменения аудиофайла. Этот флажок можно снять для принудительного перерасчёта, например после переключения Fast → Accurate. Jobs показывает режим, фактическое время анализа и факт cache hit.',
          'Stage 19.5 добавляет Adaptive: CCML сначала анализирует Fast-фрагмент и принимает его, если BPM/Key пригодны и key-strength выше настраиваемого порога (по умолчанию 0.62). Иначе автоматически выполняется Accurate по полному треку.',
          'После «Найти метаданные» CCML сохраняет provider-only BPM/Key consensus DJ Pool. Если минимум два DJ Pool источника конфликтуют с Fast BPM/Key или дают half/double BPM, Adaptive выполняет Accurate. Внешний consensus используется только как сигнал проверки и никогда автоматически не переписывает теги.',
          'Кэш Stage 19.5 привязан к точному профилю анализа: режиму, Fast-window, confidence threshold и DJ Pool hint. Поэтому результат Fast не маскируется как Accurate, а изменение настроек или consensus автоматически инвалидирует Skip unchanged.',
          'Stage 19.6 включает lean profile для официального essentia_streaming_extractor_music. CCML передаёт extractor YAML-профиль третьим аргументом, отключает frame output/high-level classifiers, сокращает ненужную low-level агрегацию и в Fast/Adaptive просит Essentia анализировать центральный временной диапазон напрямую. Это убирает обязательное предварительное FFmpeg→WAV преобразование на совместимых файлах.',
          'Если установленная Essentia не принимает profile/slice или не декодирует файл, CCML автоматически возвращается к проверенному FFmpeg WAV Fast-пути Stage 19.4. Accurate также сначала пробует full-track lean profile и при несовместимости возвращается к обычному extractor + decoder fallback. В Jobs видно engine: lean profile, FFmpeg WAV или direct.',
          'Stage 19.6 не добавляет и не распространяет бинарники Essentia: используется тот же отдельно установленный AGPLv3 extractor. Cache profile поднят до essentia-v4, поэтому старые измерения один раз пересчитываются новым engine.',
          'Stage 19.7 добавляет DJ Mix Planner в раздел Инструменты. Если выбрано 2+ трека, план строится только по выделению; при 0–1 выбранном треке используется вся медиатека, а единственный выбранный трек становится стартовым.',
          'Planner строит последовательность по BPM и Camelot: учитывает допустимый процент изменения темпа, same/adjacent/relative Camelot-переходы, half/double BPM, выбранное направление BPM и опционально избегает одного исполнителя подряд. Алгоритм ничего не записывает в файлы и не меняет теги.',
          'В плане видны score каждого перехода, adjusted BPM, предупреждения и треки без BPM/Key. Любой пункт можно сделать новым стартом или открыть в медиатеке. Для построения последовательности BPM обязателен; Key улучшает harmonic score, но не является обязательным.',
          'Stage 19.8 добавляет Quality Layer: Lookahead 1–4 оценивает не только следующий трек, но и несколько будущих переходов с ограниченным deterministic search. Значение 3 используется по умолчанию и обычно уменьшает локально хорошие, но тупиковые переходы.',
          'Опции Genre continuity и Energy flow добавляют отдельные компоненты score. Жанровая близость вычисляется по нормализованным genre tokens. Energy — это прозрачный proxy из нормализованного BPM и, когда есть измерение, Integrated LUFS; это не отдельный Essentia mood/energy classifier.',
          'Кнопка «Зафиксировать позицию» закрепляет трек на текущем месте. Planner резервирует такие треки для их позиций и оптимизирует остальные участки вокруг них; некорректные или недоступные фиксации игнорируются и считаются отдельно. Плохой forced transition явно помечается предупреждением.',
          'Stage 19.9 сохраняет именованные снимки DJ Mix Planner в SQLite-библиотеке CCML. «Сохранить» обновляет открытый план, «Сохранить как новый» создаёт отдельную копию, а список сохранённых планов после перезапуска восстанавливает точный snapshot маршрута, набор треков, настройки Planner и pinned positions.',
          'Открытый snapshot не пересчитывается скрытно. Нажмите «Построить план», если нужно заново оптимизировать маршрут по текущим данным медиатеки, сохранив загруженные scope/options. Удаление удаляет только запись Planner — аудиофайлы и теги не меняются.',
          'Экспорт M3U8 создаёт extended UTF-8 playlist с путями файлов и EXTINF. Экспорт CSV создаёт UTF-8 таблицу для Excel с порядком, Artist/Title, исходным и adjusted BPM, Camelot/Open Key, genre/energy, transition score и путём файла. Для обоих форматов используется нативный Save dialog.',
          'Stage 20.0 превращает результат в редактируемый DJ Set Timeline. Незаблокированные строки можно перетаскивать мышью или двигать кнопками ↑/↓. Заблокированные позиции остаются на месте, а остальные свободные треки переставляются вокруг них. После каждой ручной перестановки CCML заново рассчитывает score BPM/Camelot/genre/energy для фактической последовательности.',
          'Время таймлайна — это накопленная длительность исходных треков, а не прогноз реального crossfade или времени выступления. Transition note относится к входящему переходу этой строки; Cue note — свободная заметка для трека. Заметки и блокировки входят в сохранённый snapshot и восстанавливаются после перезапуска.',
          'Экспорт M3U8 Stage 20.0 добавляет CCML-комментарии timeline/lock/cue/transition, а CSV — Timeline Start/End, Locked, Transition Note и Cue Note. Ручное редактирование по-прежнему не изменяет аудиофайлы и теги.',
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'Hotfix 19.2.1 handles files that Essentia AudioLoader can misread as silence after MP3/AAC decode errors. On AudioLoader/Error reading frame/completely silent file failures, CCML decodes the source once with its configured FFmpeg into a temporary 44.1 kHz mono PCM WAV and retries Essentia. The source file is never modified and the temporary WAV is removed after analysis.',
          'Stage 19.3 adds DJ library tools: separate sortable BPM, Musical Key and Camelot/Open Key columns, a dedicated Camelot filter, and an updated DJ table preset.',
          'Inspector → Analysis shows BPM, the source musical key, Camelot/Open Key and a conservative compatibility set: the current Camelot key, adjacent wheel numbers on the same side and relative major/minor on the opposite side. Find harmonic matches clears text search, applies that Camelot set to the full library and sorts results by Camelot → BPM.',
          'Stage 19.4 speeds up Essentia: Fast mode defaults to the center 120 seconds of long tracks decoded through FFmpeg, while the Jobs queue analyzes 2 tracks in parallel. Settings exposes Fast/Accurate, 1–4 workers and a 30–300 second Fast window.',
          'Batch analysis skips unchanged files by default when the stored Essentia measurement is newer than the audio file modification time. Disable that option to force recomputation, for example after switching Fast → Accurate. Jobs shows mode, actual analysis time and cache hits.',
          'Stage 19.5 adds Adaptive mode: CCML analyzes the Fast window first and accepts it when BPM/Key are usable and key strength meets the configurable threshold (0.62 by default). Otherwise it automatically escalates to Accurate full-track analysis.',
          'After Find metadata, CCML persists provider-only DJ Pool BPM/Key consensus. If at least two DJ Pool sources conflict with Fast BPM/Key or indicate a half/double BPM relation, Adaptive escalates to Accurate. External consensus is only a verification hint and never writes tags automatically.',
          'Stage 19.5 cache identity includes the exact analysis mode, Fast window, confidence threshold and DJ Pool hint. A Fast result cannot masquerade as Accurate, and changing settings or consensus automatically invalidates Skip unchanged.',
          'Stage 19.6 enables a lean profile for the official essentia_streaming_extractor_music. CCML passes a YAML profile as the extractor third argument, disables frame output/high-level classifiers, reduces unused low-level aggregation and asks Essentia to analyze the center time range directly in Fast/Adaptive. Compatible files no longer require a preliminary FFmpeg-to-WAV conversion.',
          'If the installed Essentia build rejects profile/slicing or cannot decode a file, CCML automatically falls back to the proven Stage 19.4 FFmpeg WAV Fast path. Accurate also tries a full-track lean profile first and falls back to the normal extractor plus decoder recovery. Jobs reports the engine as lean profile, FFmpeg WAV or direct.',
          'Stage 19.6 does not add or redistribute Essentia binaries: it uses the same separately installed AGPLv3 extractor. The cache profile is bumped to essentia-v4, so older measurements are recalculated once under the new engine.',
          'Stage 19.7 adds DJ Mix Planner under Tools. With 2+ selected tracks the plan is limited to the selection; with 0–1 selected tracks it uses the whole library and a single selected track becomes the starting point.',
          'The planner orders tracks using BPM and Camelot: maximum tempo-shift percentage, same/adjacent/relative Camelot transitions, half/double BPM, an optional BPM direction and an optional same-artist penalty. It never writes files or changes tags.',
          'Each step shows transition score, adjusted BPM and warnings plus the number of tracks missing BPM/Key. Any planned track can become the new start or be revealed in the library. BPM is required for planning; Key improves the harmonic score but is optional.',
          'Stage 19.8 adds a Quality Layer: Lookahead 1–4 scores not only the immediate next track but several future transitions through a bounded deterministic search. The default is 3, which reduces locally attractive choices that lead into poor follow-up transitions.',
          'Genre continuity and Energy flow add separate score components. Genre similarity uses normalized genre tokens. Energy is an explicit proxy derived from normalized BPM plus Integrated LUFS when that measurement exists; it is not a separate Essentia mood/energy classifier.',
          'Pin position fixes a track at its current place. The planner reserves pinned tracks for those positions and optimizes the remaining route around them; invalid or unavailable pins are ignored and counted. Weak forced transitions are shown with an explicit warning.',
          'Stage 19.9 persists named DJ Mix Planner snapshots in the CCML SQLite library. Save updates the open plan, Save as new creates a separate copy, and the saved-plan selector restores the exact route snapshot, scope, planner controls and pinned positions after a restart.',
          'A reopened snapshot is not silently recalculated. Press Build plan when you want to regenerate it against current library metadata while keeping the saved scope/options. Delete removes only the saved planner record; audio files and tags are untouched.',
          'Export M3U8 writes an extended UTF-8 playlist with track paths and EXTINF labels. Export CSV writes a UTF-8 Excel-friendly table containing order, artist/title, original and adjusted BPM, Camelot/Open Key, genre/energy, transition score and source path. Both exports use the native Save dialog.',
          'Stage 20.0 turns the result into an editable DJ Set Timeline. Drag unlocked rows, or use ↑/↓, to change the exact sequence. Locked positions remain fixed while the other unlocked tracks move around them. After every manual reorder CCML recalculates BPM/Camelot/genre/energy transition scores for the resulting order instead of keeping stale Planner scores.',
          'Timeline times are cumulative source-track durations, not predicted crossfade or performance times. Transition note belongs to the incoming transition for that row; Cue note is free text for the track. Notes and locks are stored inside saved snapshots and restored after restart.',
          'Stage 20.0 M3U8 export adds CCML timeline/lock/cue/transition comment lines, while CSV adds timeline start/end, Locked, Transition Note and Cue Note columns. Manual editing remains read-only with respect to source audio and tags.',
