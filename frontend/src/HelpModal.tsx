    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'Stage 16.4 добавляет «Проверить аудио». FFmpeg декодирует все файлы текущей группы в mono PCM 4 kHz и сравнивает нормализованные признаки энергии, roughness и zero-crossing. После Hotfix 18.6.6 выравнивание остаётся покадровым в пределах ±6 секунд для близких длительностей и адаптивно расширяется до ±60 секунд для Intro/Edit/Extended-различий. Результаты: «Совпадает», «Похоже», «Отличается» или «Ошибка».',
          ...
          'Hotfix 18.6.1 привязывает результат «Проверить аудио» к точному составу группы. Group key теперь содержит весь отсортированный набор track ID, а UI принимает и показывает verification только если совпали и groupKey, и полный набор comparison track ID. Завершившийся после пересканирования старый запрос больше не может показать Same/Similar у другой группы.',
          'Hotfix 18.6.2 усиливает «Проверить аудио» спектральным признаком. Помимо energy/roughness/zero-crossing CCML строит gain-independent профиль энергии на 8 частотах от 80 до 1750 Hz и учитывает его в similarity. Это снижает риск ложного Similar для разных треков с похожей огибающей и темпом, но разным тональным/тембральным содержимым.',
          'Hotfix 18.6.5 добавляет абсолютную защиту по длительности для вердикта Similar: даже при очень высоком feature similarity разница полной длительности больше 60 секунд даёт Different. Same по-прежнему требует разницу не более 1.5 секунды.',
          'Hotfix 18.6.6 устраняет фиксированное окно ±6 секунд: максимальный поиск сдвига теперь зависит от разницы длины признаков и ограничен ±60 секундами. При широком диапазоне выполняется coarse scan с шагом 500 мс и точная доработка вокруг лучшего сдвига с шагом 100 мс. Корреляция считается относительно среднего внутри фактически сравниваемого окна, поэтому длинное вступление не искажает нормализацию совпадающей основной части.',
          'Hotfix 18.6.10 исправляет сравнение silence/near-silence файлов. CCML хранит RMS и peak исходного decoded PCM: если оба файла ниже консервативных порогов тишины, их content similarity считается 1.0, после чего обычные ограничения длительности решают Same/Similar/Different. Если тихим является только один файл, similarity остаётся 0, чтобы не создавать опасное ложноположительное совпадение.',
          ...
          'Hotfix 18.6.7 синхронизирует оставшийся legacy SearchEnrichment pipeline с политикой провайдеров: MUZVIZOR теперь относится к deferred-группе, потому что его public genre fallback сериализуется через mutex и может иметь повышенную задержку. Текущий основной путь «Дополнить данные» после Stage 18.0.1 по-прежнему использует общий metadataService.Search с 25-секундным provider timeout.',
          ...
          'Hotfix 18.6.4 исправляет MUZVIZOR query escaping и уточняет старый комментарий: значения query для /tracks?query= и /api/v1/tracks/?query= теперь кодируются как query data через QueryEscape, затем + для пробелов переводится в %20. Поэтому & и другие query-разделители больше не могут разорвать Artist/Title запрос. Metadata-cache повышен до v14.',
          ...
          'Hotfix 18.2.5 добавляет диагностическое логирование MUZVIZOR. При CCML_MUZVIZOR_DEBUG=1 консоль показывает URL/HTTP результат, размер HTML, наличие Artist/Title и track__row_main/колонок, краткий preview, сырые и отфильтрованные кандидаты и каждую fallback-страницу. Версия metadata-cache повышена до v5, чтобы диагностика получила свежий запрос.',
          'Hotfix 18.2.6 переключает MUZVIZOR на подтверждённый браузерный JSON endpoint /api/v1/tracks/?query= как основной источник. Запрос выполняется без browser Cookie/Authorization; из JSON нормализуются Artist, Title, BPM, Camelot Key, Genre и DJ Stage. HTML/genre parsing остаётся только fallback, diagnostics сохранена, metadata-cache повышен до v6.',
          'Hotfix 18.6.8 ограничивает рекурсивный разбор MUZVIZOR JSON глубиной 16 уровней. Ограничение применяется к сбору вложенных объектов и к recursive readers для text/BPM/Camelot, поэтому чрезмерно вложенный или враждебный публичный ответ безопасно игнорируется вместо неограниченного роста стека. Обычная неглубокая структура API остаётся без изменений; metadata-cache повышен до v15.',
          'Stage 18.3 добавляет Bananastreet как опциональный DJ Pool источник через подтверждённую публичную страницу /search?q=. Провайдер извлекает Artist, Title и Genre из публичных карточек и консервативно фильтрует их по запросу.',
          'Подтверждённые публичные карточки Bananastreet не показывают BPM/Key, поэтому CCML не выдумывает эти значения и оставляет их пустыми. Авторизация, скрытые API, прослушивание и скачивание аудио не используются; Stage 17.1 fallback и общий scoring/Merge применяются автоматически.',
          'Hotfix 18.3.1 приводит Bananastreet-запрос к браузерному виду Artist - Title с %20 вместо + и добавляет диагностику CCML_BANANASTREET_DEBUG=1: URL/HTTP результат, размер HTML, наличие Artist/Title, соседние видимые строки, сырые карточки и итоговый match score. Metadata-cache повышен до v8 для свежего запроса.',
          'Hotfix 18.3.2 учитывает фактический compact-формат результатов поиска Bananastreet: одна строка Artist - Title, после которой может идти имя загрузившего пользователя. CCML перебирает возможные разделители, выбирает только строго совпавшую пару Artist/Title и не принимает uploader за артиста. Старый card-parser остаётся fallback; неподтверждённые Genre/BPM/Key для compact-строки остаются пустыми. Metadata-cache повышен до v9.',
          'Hotfix 18.6.3 исправляет экранирование Bananastreet search query: значение q теперь кодируется как query-параметр, а не как path segment. Символы вроде & больше не разрывают запрос (например, Simon & Garfunkel кодируется как Simon%20%26%20Garfunkel), при этом пробелы сохраняются в браузерном виде %20. Metadata-cache повышен до v13.',
          'Stage 18.4 добавляет Mixcloud через официальный публичный read-only API /search/?q=...&type=cloudcast. Чтение API не требует OAuth; CCML не выполняет write-действия и не получает аудиопотоки.',
          'Mixcloud — источник DJ/show metadata, а не трековый каталог. Поэтому CCML принимает результат только когда cloudcast name содержит сильное Artist - Title совпадение либо uploader действительно совпадает с Artist. Из tags в Genre попадают только известные музыкальные жанры; upload date, show duration, artwork, BPM/Key и прочие show-поля не подменяют трековые метаданные.',
          'Для диагностики Stage 18.4 поддерживает CCML_MIXCLOUD_DEBUG=1: в консоль выводятся URL, HTTP результат, число JSON-результатов, исходные cloudcast name/user/tags, принятые кандидаты и их fit. Metadata-cache повышен до v10.',
          'Stage 18.5 добавляет Jestei Pool как публичный DJ Pool источник. CCML использует /search?q= для получения публичных /track/<id> ссылок и читает только публично видимые Artist, Title, Genre, BPM, Camelot Key (если присутствует) и часть ночи/Stage.',
          'Авторизация Jestei в Stage 18.5 намеренно не эмулируется: публичной документации внешнего OAuth/API-token способа не подтверждено, поэтому CCML не хранит пароль Jestei и не копирует browser Cookie. Авторизацию можно добавить отдельным обновлением после подтверждения поддерживаемого способа.',
          'CCML_JESTEI_DEBUG=1 включает диагностику Jestei: search URL, HTTP, найденные track URL, размер detail HTML, разобранные поля, отклонённые страницы и итоговый fit. Metadata-cache повышен до v11.',
          'Stage 18.6 добавляет DJ Pool Quality Layer. Совпадающие Artist + полный versioned Title от разных DJ Pool источников сохраняются как evidence для Merge, но в основном списке кандидатов показываются одной строкой; Extended/Radio/Remix версии не склеиваются.',
          'Для Genre/BPM/Key CCML теперь учитывает надёжность источника и межисточниковый consensus. Совпадающие значения объединяются в один Merge option с количеством поддержавших источников; BPM в пределах ±0.5 считается одним измерением. Jestei/MUZVIZOR/RemixPool имеют более высокий вес для BPM/Key, а Mixcloud не получает бонус Genre как show/tag источник.',
          'Если DJ Pool источники расходятся по Genre, BPM или Key, Metadata Merge явно показывает конфликт и количество разных значений/источников. Автоматический CCML Merge выбирает option по confidence + source reliability + bounded consensus bonus. Metadata-cache повышен до v12.',
          ...
          'Hotfix 18.6.9 исправляет навигацию «Следующая необработанная»: перед переходом очищается поисковая строка и включается общий список. Поэтому поисковый фильтр больше не может скрыть целевую группу и заставить filtered-list effect молча выбрать другую группу.',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Stage 19.0 делает Essentia полноценным внешним инструментом CCML: в Настройках можно выбрать essentia_streaming_extractor_music, путь сохраняется в пользовательской конфигурации и имеет приоритет над CCML_ESSENTIA / PATH. Кнопка «Автопоиск» удаляет сохранённый override.',
          'Essentia распространяется отдельно под AGPLv3 и не встраивается в MIT-дистрибутив CCML; кнопка «Официальная загрузка» открывает страницу загрузок проекта.',
          'Stage 19.1 добавляет массовый BPM/Key-анализ выбранных треков и всей медиатеки через постоянную очередь Задач. Для каждого трека сохраняются BPM, тональность, major/minor scale и key-strength confidence; pause/resume/cancel/retry работают через общий job manager.',
          'Опция записи тегов использует обычный Tag History/Undo CCML. Режим «только отсутствующие» не перезаписывает уже заполненные BPM/Key/KeyScale; без записи тегов результат обновляет только локальную базу CCML и может быть заменён при следующем сканировании файла.',
          'Stage 19.2 переводит результат Essentia в DJ-нотации Camelot и Open Key и сохраняет последнее локальное измерение отдельно от тегов. «Найти метаданные» сравнивает его с BPM/Key-консенсусом источников типа DJ Pool, показывает совпадение, half/double-tempo, relative major/minor или конфликт и даёт рекомендацию без скрытой автозамены.',
          'Если локальная Essentia совпадает с внешним значением, она учитывается как дополнительное подтверждение в списке вариантов Merge. При конфликте локальное значение остаётся отдельным вариантом «локальная Essentia».',
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: [
          ...
          'Stage 16.4 adds Check audio. FFmpeg decodes every file in the current group to 4 kHz mono PCM and compares normalized energy, roughness and zero-crossing features. After Hotfix 18.6.6, alignment remains frame-by-frame within ±6 seconds for near-equal lengths and expands adaptively up to ±60 seconds for Intro/Edit/Extended differences. Results are Same, Similar, Different or Error.',
          ...
          'Hotfix 18.6.1 binds each Check audio result to the exact duplicate-group membership. Group keys now contain the full sorted track-ID set, and the UI accepts/renders a verification only when both groupKey and the complete comparison track-ID set match. A stale request finishing after a rescan can no longer display Same/Similar for another group.',
          'Hotfix 18.6.2 strengthens Check audio with spectral evidence. In addition to energy/roughness/zero-crossing, CCML builds a gain-independent 8-frequency energy profile from 80 to 1750 Hz and includes it in similarity. This reduces false Similar results for different tracks that share a comparable envelope and tempo but have different tonal/timbral content.',
          'Hotfix 18.6.5 adds an absolute duration guard for the Similar verdict: even with very high feature similarity, a full-duration difference above 60 seconds is Different. Same continues to require a difference of at most 1.5 seconds.',
          'Hotfix 18.6.6 removes the fixed ±6 second alignment window: the shift range now grows with the feature-length difference and is capped at ±60 seconds. Wide searches use a 500 ms coarse scan followed by 100 ms frame-level refinement around the best shift. Correlation is centered within the actual overlap window, so a long intro no longer distorts normalization of the matching main section.',
          'Hotfix 18.6.10 fixes silence/near-silence comparison. CCML keeps RMS and peak levels from the decoded PCM: when both files remain below conservative silence thresholds, content similarity is treated as 1.0 and the normal duration guards then decide Same/Similar/Different. If only one file is silent, similarity remains 0 to avoid an unsafe false-positive match.',
          ...
          'Hotfix 18.6.7 brings the remaining legacy SearchEnrichment pipeline in line with provider policy: MUZVIZOR is now deferred because its public genre fallback is serialized by a mutex and can have higher latency. The current Enrich metadata path introduced by Stage 18.0.1 still uses the shared metadataService.Search path with a 25-second provider timeout.',
          ...
          'Hotfix 18.6.4 fixes MUZVIZOR query escaping and corrects the old explanatory comment: query values for /tracks?query= and /api/v1/tracks/?query= are now encoded as query data with QueryEscape, then space plus signs are converted to %20. Characters such as & can no longer split the Artist/Title query. The metadata cache is bumped to v14.',
          ...
          'Hotfix 18.2.5 adds MUZVIZOR diagnostic logging. With CCML_MUZVIZOR_DEBUG=1 the console reports URL/HTTP results, HTML size, Artist/Title and track__row_main/column presence, a compact preview, raw and filtered candidates, and every fallback page. The metadata-cache version is bumped to v5 so diagnostics run on a fresh lookup.',
          'Hotfix 18.2.6 switches MUZVIZOR to the confirmed browser JSON endpoint /api/v1/tracks/?query= as the primary source. The request sends no browser Cookie/Authorization state; Artist, Title, BPM, Camelot Key, Genre and DJ Stage are normalized from JSON. HTML/genre parsing remains only as a fallback, diagnostics stay available, and the metadata cache is bumped to v6.',
          'Hotfix 18.6.8 caps recursive MUZVIZOR JSON traversal at 16 levels. The limit applies to nested-object collection and recursive text/BPM/Camelot readers, so an excessively nested or hostile public response is ignored safely instead of allowing unbounded stack growth. Normal shallow API responses are unchanged; the metadata cache is bumped to v15.',
          'Stage 18.3 adds Bananastreet as an optional DJ Pool source through the confirmed public /search?q= page. The provider extracts Artist, Title and Genre from public cards and conservatively filters them against the query.',
          'The confirmed public Bananastreet cards do not expose BPM/Key, so CCML does not invent those values and leaves them empty. No login, hidden API, playback or audio download is used; Stage 17.1 fallback and the shared scoring/Merge pipeline apply automatically.',
          'Hotfix 18.3.1 makes Bananastreet queries browser-compatible as Artist - Title with %20 rather than + and adds CCML_BANANASTREET_DEBUG=1 diagnostics: URL/HTTP result, HTML size, Artist/Title presence, nearby visible lines, raw cards and final match scores. The metadata cache is bumped to v8 for a fresh request.',
          'Hotfix 18.3.2 supports the observed compact Bananastreet search-result format: a single Artist - Title line followed by an optional uploader name. CCML tries each possible separator, accepts only a strong Artist/Title match, and never treats the uploader as the full artist. The older card parser remains as a fallback; unconfirmed Genre/BPM/Key stay empty for compact rows. The metadata cache is bumped to v9.',
          'Hotfix 18.6.3 fixes Bananastreet search-query escaping: q is now encoded as a query parameter instead of a path segment. Characters such as & no longer split or truncate the request (for example Simon & Garfunkel becomes Simon%20%26%20Garfunkel), while spaces remain browser-style %20. The metadata cache is bumped to v13.',
          'Stage 18.4 adds Mixcloud through the official public read-only /search/?q=...&type=cloudcast API. API reads require no OAuth; CCML performs no write actions and does not access audio streams.',
          'Mixcloud is a DJ/show metadata source rather than a track catalog. CCML therefore accepts a result only when the cloudcast name contains a strong Artist - Title identity or the uploader itself strongly matches Artist. Only recognized music tags become Genre; upload date, show duration, artwork, BPM/Key and other show-level fields are not substituted for track metadata.',
          'Stage 18.4 also supports CCML_MIXCLOUD_DEBUG=1 diagnostics: the console reports URL/HTTP status, JSON result count, raw cloudcast name/user/tags, accepted candidates and fit. The metadata cache is bumped to v10.',
          'Stage 18.5 adds Jestei Pool as a public DJ Pool source. CCML uses /search?q= to discover public /track/<id> pages and reads only publicly visible Artist, Title, Genre, BPM, Camelot Key when exposed, and the DJ night stage.',
          'Stage 18.5 deliberately does not emulate Jestei login: no supported external OAuth/API-token integration has been confirmed, so CCML does not store a Jestei password or copy browser Cookies. Authentication can be added separately after a supported integration method is confirmed.',
          'CCML_JESTEI_DEBUG=1 enables Jestei diagnostics: search URL, HTTP result, discovered track URLs, detail HTML size, parsed fields, rejected pages and final fit. The metadata cache is bumped to v11.',
          'Stage 18.6 adds the DJ Pool Quality Layer. Matching Artist + full versioned Title results from different DJ Pool sources remain as evidence for Merge but collapse to one row in the visible candidate list; Extended/Radio/Remix versions remain separate.',
          'For Genre/BPM/Key, CCML now combines source reliability with cross-provider consensus. Equal values become one Merge option with the number of supporting sources; BPM values within ±0.5 are treated as one measurement. Jestei/MUZVIZOR/RemixPool receive stronger BPM/Key weights, while Mixcloud gets no Genre bonus because it is a show/tag source.',
          'When DJ Pool sources disagree on Genre, BPM or Key, Metadata Merge explicitly shows a conflict with the number of distinct values and sources. Automatic CCML Merge selects by confidence + source reliability + a bounded consensus bonus. The metadata cache is bumped to v12.',
          ...
          'Hotfix 18.6.9 fixes Next unresolved navigation: it clears the search query and switches to the complete list before selecting the target. A search filter can no longer hide that target and let the filtered-list effect silently replace it with another group.',
      ...
      },
      {
        ...
        bullets: [
          ...
          'Stage 19.0 makes Essentia a first-class external CCML tool: Settings can select essentia_streaming_extractor_music, the path is persisted in the user configuration and takes priority over CCML_ESSENTIA / PATH. Use auto-detection removes the saved override.',
          'Essentia is distributed separately under AGPLv3 and is not bundled into the MIT-licensed CCML distribution; Official download opens the project download page.',
          'Stage 19.1 adds batch BPM/Key analysis for selected tracks and the entire library through the persistent Jobs queue. Each item reports BPM, musical key, major/minor scale and key-strength confidence; pause/resume/cancel/retry use the shared job manager.',
          'Write tags uses the normal CCML Tag History/Undo path. Only missing preserves existing BPM/Key/KeyScale fields; with tag writing disabled the result updates only the local CCML database and can be replaced by a later file scan.',
          'Stage 19.2 converts Essentia output to Camelot and Open Key notation and persists the latest local measurement separately from file tags. Find metadata compares it with BPM/Key consensus from DJ Pool providers, labels exact agreement, half/double tempo, relative major/minor or conflict, and recommends a source without silently replacing the normal suggestion.',
          'When local Essentia agrees with an external value it is counted as additional evidence in Merge. When it conflicts, the local result remains a separate Essentia local choice.',
