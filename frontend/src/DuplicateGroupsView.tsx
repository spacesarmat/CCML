...
function DuplicateGroupsView({
  ...
  const copy = language === 'ru'
    ? {
        ...
        verifyAudioHint: 'FFmpeg декодирует файлы в низкочастотный mono PCM и сравнивает форму сигнала вместе с грубым спектральным профилем. Два silence/near-silence файла распознаются отдельно по RMS/peak; тихий файл не считается совпадением с обычным аудио. Выравнивание адаптивно расширяется до 60 секунд для Intro/Edit/Extended-различий; Similar также требует разницу полной длительности не более 60 секунд. Это дополнительная эвристика, а не доказательство идентичности.',
    ...
    : {
        ...
        verifyAudioHint: 'FFmpeg decodes files to low-rate mono PCM and compares waveform features together with a coarse spectral profile. Two silence/near-silence files are handled separately using RMS/peak levels; a quiet file is never treated as a match for normal audible audio. Alignment expands adaptively up to 60 seconds for Intro/Edit/Extended differences; Similar also requires the full-duration difference to stay within 60 seconds. This is an additional heuristic, not proof of identity.',
